...
export default function NewFirmRegistrationPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <NewFirmRegistrationContent />
    </Suspense>
  );
}

function NewFirmRegistrationContent() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      fetch(`/api/admin/service-requests/details?id=${editId}&type=NEW_FIRM_REGISTRATION`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setFormValues({
    ...
    for (const file of files) {
      ...
      const res = await fetch("/api/documents/upload", {
        ...
        body: JSON.stringify({
          ...
          clientEmail: email,
          title: `New Firm Reg Doc: ${file.name}`,
        }),
      });
      ...
      if (result.success) {
        if (editId && srId) {
          await fetch("/api/client/resubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: srId, clientNotes: "Firm registration details updated." })
          });
        }
      ...
      <main className="main-content">
        <div className="page">
          <div className="form-container">
            <h2>{editId ? "Edit" : "New"} Firm Registration</h2>
