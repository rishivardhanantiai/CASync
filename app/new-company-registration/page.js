...
export default function NewCompanyRegistrationPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <NewCompanyRegistrationContent />
    </Suspense>
  );
}

function NewCompanyRegistrationContent() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      fetch(`/api/admin/service-requests/details?id=${editId}&type=NEW_COMPANY_REGISTRATION`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setFormValues({
              requiredName: data.data.companyName || "",
              contactPerson: data.data.contactPerson || "",
              address: data.data.address || "",
              mobileNo: data.data.phone || "",
              emailId: data.data.email || "",
              businessToStart: data.data.businessToStart || ""
            });
    ...
    for (const file of files) {
      ...
      const res = await fetch("/api/documents/upload", {
        ...
        body: JSON.stringify({
          ...
          clientEmail: email,
          title: `New Company Reg Doc: ${file.name}`,
        }),
      });
      ...
      if (result.success) {
        if (editId && srId) {
          await fetch("/api/client/resubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: srId, clientNotes: "New company registration details updated." })
          });
        }
      ...
      <main className="main-content">
        <div className="page">
          <div className="form-container">
            <h2>{editId ? "Edit" : "New"} Company Registration</h2>
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                <label htmlFor="requiredName">Required Company Name *</label>
                <input
                  type="text"
                  id="requiredName"
                  required
                  value={formValues.requiredName}
                  onChange={(e) => setFormValues(prev => ({ ...prev, requiredName: e.target.value }))}
                />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                <label htmlFor="address">Required Registration Address *</label>
                <input
                  type="text"
                  id="address"
                  required
              ...
              <div className="form-group">
                <label htmlFor="mobileNo">Contact Person Mobile Number *</label>
                <input
                  type="tel"
                  id="mobileNo"
                  required
                  ...
                  onChange={(e) => setFormValues(prev => ({ ...prev, mobileNo: e.target.value }))}
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setFormValues(prev => ({ ...prev, emailId: e.target.value }))}
                />
              </div>
              <div className="form-group">
                <label htmlFor="businessToStart">Business they want to start *</label>
                <input
                  type="text"
                  id="businessToStart"
                  required
                  value={formValues.businessToStart}
                  onChange={(e) => setFormValues(prev => ({ ...prev, businessToStart: e.target.value }))}
                />
              </div>
              ...
              <div className="button-group">
                ...
                <button type="submit" disabled={!agreed} style={{ opacity: agreed ? 1 : 0.6, cursor: agreed ? "pointer" : "not-allowed" }}>
                  {editId ? "Update & Resubmit" : "Register"}
                </button>
              </div>
            </form>
          </div>
