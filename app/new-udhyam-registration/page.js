...
export default function NewUdhyamRegistrationPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <NewUdhyamRegistrationContent />
    </Suspense>
  );
}

function NewUdhyamRegistrationContent() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      fetch(`/api/admin/service-requests/details?id=${editId}&type=NEW_UDHYAM_REGISTRATION`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setFormValues({
              requiredName: data.data.requiredName || "",
    ...
    for (const file of files) {
      ...
      const res = await fetch("/api/documents/upload", {
        ...
        body: JSON.stringify({
          ...
          clientEmail: email,
          title: `New Udhyam Reg Doc: ${file.name}`,
        }),
      });
      ...
      if (result.success) {
        if (editId && srId) {
          await fetch("/api/client/resubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: srId, clientNotes: "Udhyam registration details updated." })
          });
        }
      ...
      <main className="main-content">
        <div className="page">
          <div className="form-container">
            <h2>{editId ? "Edit" : "New"} Udhyam Registration</h2>
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                <label htmlFor="requiredName">Required Firm Name *</label>
                <input
                  type="text"
                  id="requiredName"
                  required
