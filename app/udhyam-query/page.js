...
export default function UdhyamQueryPage() {
  return (
...
function UdhyamQueryContent() {
  ...
  useEffect(() => {
    ...
    if (currentUser?.email) {
      ...
      router.replace("/login?redirect=/udhyam-query");
    } else {
      router.replace("/login?redirect=/udhyam-query");
    }

    if (editId) {
      fetch(`/api/admin/service-requests/details?id=${editId}&type=UDHYAM_QUERY`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setFormValues({
              ...
              udhyamNo: data.data.udhyamNo || "",
              queryType: data.data.queryType || "Udhyam Query",
      ...
      if (result.success) {
        if (editId && srId) {
          await fetch("/api/client/resubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: srId, clientNotes: "Udhyam query updated." })
          });
        }
      ...
      <main className="main-content">
        <div className="page">
          <div className="form-container">
            <h2>{editId ? "Edit" : "New"} Udhyam Query</h2>
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                <label htmlFor="udhyamNo">Udhyam Registration Number *</label>
                <input
                  type="text"
                  id="udhyamNo"
                  required
                  placeholder="UDYAM-XX-00-0000000"
                  value={formValues.udhyamNo}
                  onChange={(e) => setFormValues(prev => ({ ...prev, udhyamNo: e.target.value.toUpperCase() }))}
                />
              </div>
              <div className="form-group">
