...
export default function GstQueryPage() {
  return (
...
function GstQueryContent() {
  ...
  useEffect(() => {
    ...
    if (currentUser?.email) {
      ...
      // Pre-fill email and name if new query
      if (!editId) {
        setFormValues(prev => ({
          ...prev,
      ...
      router.replace("/login?redirect=/gst-query");
    } else {
      router.replace("/login?redirect=/gst-query");
    }

    if (editId) {
      fetch(`/api/admin/service-requests/details?id=${editId}&type=GST_QUERY`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setFormValues({
              ...
              queryType: data.data.queryType || "GST Query",
    ...
    try {
      const res = await fetch("/api/client/submit-form", {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...formValues,
            userEmail // used to match the client user record
          }
        })
      });
      ...
      if (result.success) {
        if (editId && srId) {
          await fetch("/api/client/resubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: srId, clientNotes: "GST query updated." })
          });
        }
      ...
      <main className="main-content">
        <div className="page">
          <div className="form-container">
            <h2>{editId ? "Edit" : "New"} GST Query</h2>
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  title="Valid 15-digit GSTIN format"
                  value={formValues.gstNumber}
                  onChange={(e) => setFormValues(prev => ({ ...prev, gstNumber: e.target.value.toUpperCase() }))}
                />
              </div>
              <div className="form-group">
