...
export default function IncomeTaxQueryPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <IncomeTaxQueryContent />
    </Suspense>
  );
}

function IncomeTaxQueryContent() {
  ...
  useEffect(() => {
    ...
    if (currentUser?.email) {
      ...
      router.replace("/login?redirect=/income-tax-query");
    } else {
      router.replace("/login?redirect=/income-tax-query");
    }

    if (editId) {
      fetch(`/api/admin/service-requests/details?id=${editId}&type=IT_QUERY`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setFormValues({
      ...
      if (result.success) {
        if (editId && srId) {
          await fetch("/api/client/resubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: srId, clientNotes: "Income Tax query updated." })
          });
        }
      ...
      <main className="main-content">
        <div className="page">
          <div className="form-container">
            <h2>{editId ? "Edit" : "New"} Income Tax Query</h2>
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  title="Enter valid 10-digit PAN"
                  value={formValues.pan}
                  onChange={(e) => setFormValues(prev => ({ ...prev, pan: e.target.value.toUpperCase() }))}
                />
              </div>
              <div className="form-group">
