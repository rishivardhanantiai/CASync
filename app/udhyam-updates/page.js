...
export default function UdhyamUpdatesPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <UdhyamUpdatesContent />
    </Suspense>
  );
}

function UdhyamUpdatesContent() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      fetch(`/api/admin/service-requests/details?id=${editId}&type=UDHYAM_UPDATES`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setFormValues({
              ...
              changeDetails: data.data.changeDetails || ""
            });
    ...
    for (const file of files) {
      ...
      const res = await fetch("/api/documents/upload", {
        ...
        body: JSON.stringify({
          ...
          clientEmail: email,
          title: `Udhyam Update Doc: ${file.name}`,
        }),
      });
      ...
      if (result.success) {
        if (editId && srId) {
          await fetch("/api/client/resubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: srId, clientNotes: "Udhyam details update request modified." })
          });
        }
      ...
      <main className="main-content">
        <div className="page">
          <div className="form-container">
            <h2>{editId ? "Edit" : "Udhyam"} Updates & Modification</h2>
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                <label htmlFor="registrationNo">Udhyam Registration Number *</label>
                <input
                  type="text"
                  id="registrationNo"
                  required
                  ...
                  onChange={(e) => setFormValues(prev => ({ ...prev, registrationNo: e.target.value.toUpperCase() }))}
                />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                <label htmlFor="changeDetails">Details of Modification/Changes *</label>
                <textarea
                  id="changeDetails"
                  required
                  rows={4}
                  value={formValues.changeDetails}
                  onChange={(e) => setFormValues(prev => ({ ...prev, changeDetails: e.target.value }))}
              ...
              <div className="button-group">
                ...
                <button type="submit" disabled={!agreed} style={{ opacity: agreed ? 1 : 0.6, cursor: agreed ? "pointer" : "not-allowed" }}>
                  {editId ? "Update & Resubmit" : "Save Changes"}
                </button>
              </div>
            </form>
          </div>
