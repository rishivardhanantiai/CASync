...
export default function NewGstRegistrationPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <NewGstRegistrationContent />
    </Suspense>
  );
}

function NewGstRegistrationContent() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      fetch(`/api/admin/service-requests/details?id=${editId}&type=APPLY_NEW_GST`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setFormValues({
              requiredName: data.data.firmName || "",
              contactPerson: data.data.mainPerson || "",
              address: data.data.address || "",
              mobileNo: data.data.udyamNo || "",
              email: data.data.userEmail || "",
              businessToStart: data.data.businessType || ""
            });
    ...
    for (const file of files) {
      ...
      const res = await fetch("/api/documents/upload", {
        ...
        body: JSON.stringify({
          ...
          clientEmail: email,
          title: `New GST Reg Doc: ${file.name}`,
        }),
      });
      ...
      if (result.success) {
        if (editId && srId) {
          await fetch("/api/client/resubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: srId, clientNotes: "GST registration updated." })
          });
        }
      ...
      <main className="main-content">
        <div className="page">
          <div className="form-container">
            <h2>{editId ? "Edit" : "Apply"} New GST Registration</h2>
