...
export function applyRequestFilters(requests, filters) {
  const { search, status, serviceType, assignedTo, startDate, endDate } = filters;
  ...
  return requests.filter(r => {
    ...
    return matchSearch && matchStatus && matchService && matchAssigned && matchDate;
  });
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        {/* Status */}
        <select value={filters.status} onChange={e => set("status", e.target.value)} style={selectStyle} className="premium-filter-input">
          {STATUS_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>

        {/* Service Type */}
        <select value={filters.serviceType} onChange={e => set("serviceType", e.target.value)} style={selectStyle} className="premium-filter-input">
          {SERVICE_TYPES.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>

        {/* Assigned To (Admin only) */}
        {showAssigned && (
          <select value={filters.assignedTo} onChange={e => set("assignedTo", e.target.value)} style={selectStyle} className="premium-filter-input">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <span style={{ fontSize: "12px", color: t.muted }}>From:</span>
          <input
            type="date"
      ...
      {(active || resultCount !== undefined) && (
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center", marginTop: "10px" }}>
          ...
          {filters.startDate && (
            <Chip label={`From: ${filters.startDate}`} onRemove={() => set("startDate", "")} dark={dark} />
          )}
          {filters.endDate && (
            <Chip label={`To: ${filters.endDate}`} onRemove={() => set("endDate", "")} dark={dark} />
          )}
